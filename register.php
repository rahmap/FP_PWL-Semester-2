...
<html>
<head>
  <title>Beruk Jantan - Registrasi</title>
...
<body class="">
<?php
include 'data/koneksi.php';include 'data/komentar-conn.php';
session_start();
if (isset($_COOKIE['id'])) {
...
if (isset($_SESSION['username'])) {
      die ('<script language="javascript">
         swal({
              ...
              text: "Keluar dulu, lalu buat akun lagi"
          }).then(function() {
...
include 'data/koneksi.php';
  if (isset($_POST['submit'])) {
    ...
    $username = $_POST['username']; $username=htmlspecialchars($username); //$username=addslashes($username);
    $email = $_POST['email']; $email=htmlspecialchars($email);
    $pass = $_POST['password']; $pass=htmlspecialchars($pass);
    $pass_konfir = $_POST['password-konfir']; $pass_konfir=htmlspecialchars($pass_konfir);
    $fullname = htmlspecialchars($_POST['fullname']); $fullname = addslashes($fullname);
    //strtolower
    ...
    else if($pass != $pass_konfir){
      echo '<script language="javascript">
            swal("GAGAL!","Password Konfirmasi tidak sama");
            </script>';
    }
    else if ($error === 4) {
      echo '<script language="javascript">
        ...
        <div class="col-md-12">
          <form class="" method="POST" action=" " enctype="multipart/form-data">
            ...
            <div class="form-group"> <label>Konfirmasi Password</label>
              <input type="password" name="password-konfir" class="form-control" placeholder="Password"> </div>
